...
p_vec = [0 1.4 1.2 0.8 0.6];
     
p_N = size(p_vec,2);  % number of b sets to plot
...
title('F_1 = - F_m_a_g [sin(wt) + sin(pwt)]');
xlabel('w'); ylabel('Amplitude'); hold off;

% natural frequencies
% xline(sqrt((3+sqrt(5))*k(1)/2*m(1)),'r');
% xline(sqrt((3-sqrt(5))*k(1)/2*m(1)),'g');
% xline(sqrt((5+sqrt(5))*k(1)/2*m(1)),'b');
% xline(sqrt((5-sqrt(5))*k(1)/2*m(1)),'o');
...
figure;
t_new = 0:0.05:T*50;
F1 = -Fmag*sin(w*t_new)-Fmag*sin(0.8*w*t_new);
...
function dudt = rhs(t,u,m,k,b,Fmag,w,p)
    ...
    F1 = -Fmag*sin(w*t)-Fmag*sin(p*w*t);
    F2 = 0;
    F3 = 0;
    F4 = 0;
